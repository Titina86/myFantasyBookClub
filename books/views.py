...
#         return render(request, self.template_name, {'form': form})
#
# class WishListAddBookView(LoginRequiredMixin, CreateView):
#     model = MyBook
#     form_class = MyBookForm
#     template_name = 'books/my-book-add.html'
#
#     def post(self, request, *args, **kwargs):
#         profile = request.user.profile  # Профил на текущия потребител
#         form = self.form_class(request.POST)
#
#         if form.is_valid():
#             book_name = form.cleaned_data['title']
#             # Проверка дали книгата вече съществува
#             existing_book = MyBook.objects.filter(title__iexact= book_name, profile=profile).first()
#
#             if not existing_book:
#                 # Ако книгата не съществува, създайте нова
#                 new_book = form.save(commit=False)
#                 new_book.profile = profile  # Свързване с профила
#                 new_book.save()
#                 return redirect('profile-details')
#             else:
#                 if not existing_book.wish_list:
#                     existing_book.wish_list = True  # Поставете wish_list на True
#                     existing_book.save()
#                 return redirect('wish-list')
#
#         # Ако формата не е валидна
#         return render(request, self.template_name, {'form': form})
#
# @login_required
